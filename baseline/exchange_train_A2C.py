...
max_iter = 8640  # 10s一个时间步=8640，可能有很多空白时间点-----10min
...
# 实际网络
log_dir = "D:/exchange/train/debug-2021-7-29/A2C_train_logs/"
...
while True:
            ...
            for online_courier_count_y in range(online_courier_count_x + 1, len(online_couriers)):
                ...
                if courier_encounter_distance < 0.01 and \
                    ...
                    for last_time_encounter_event in env.encounter_exchange_event:
                        if (T - last_time_encounter_event.encounter_end_time_slot < 1) and \
        ...
        if len(cur_time_step_encounter_events) > 0:
            for i_encounter in cur_time_step_encounter_events:
                ...
                for encounter_order_id in range(len(encounter_to_exchange_order_list)):
                    ...
                    if to_exchange_order_raw_courier_id_list[encounter_order_id] == 0:
                        ...
                        if encounter_two_couriers[0].route_flag_to_be_updated == {}:
                            ...
                            for time_step_id in range(T, max(
                                ...
                                for state_change_id in range(len(encounter_two_couriers[0].route_flag_to_be_updated[time_step_id])):
                                    if encounter_two_couriers[0].route_flag_to_be_updated[time_step_id][state_change_id] == (
                        ...
                        if encounter_two_couriers[1].route_flag_to_be_updated == {}:
                            ...
                            for time_step_id in range(T, max(
                                ...
                                for change_state_id1 in range(len(encounter_two_couriers[1].route_flag_to_be_updated[time_step_id])):
                                    if encounter_two_couriers[1].route_flag_to_be_updated[time_step_id][change_state_id1] == (
                                    cur_this_order_loc_id, 0):
                                        to_exchange_order_raw_courier_cost_left_time = time_step_id - T
                                        break
                    ...
                    if to_exchange_order_raw_courier_cost_left_time <= 120:
                        continue
                    ...
                    for shunludan_id in range(len(encounter_two_couriers[0].order_list)):
                        if encounter_two_couriers[0].order_list[shunludan_id].dispatch_stage != 0:
                            ...
                            if (cur_order_next_stop_lat-cur_order_cur_lat == 0 and cur_order_next_stop_lon-cur_order_cur_lon == 0) or \
                    ...
                    if chosen_courier_id != to_exchange_order_raw_courier_id_list[encounter_order_id]:
                        ...
                        if chosen_courier_id == 0:
                            ...
                            if T + 1 not in encounter_two_couriers[0].route_flag_to_be_updated.keys() or \
                                    encounter_two_couriers[0].cur_order_num == 0 or len(encounter_two_couriers[0].route_flag_to_be_updated[T + 1]) == 0:
                                courier1_next_stop_location_lat = encounter_two_couriers[0].lat
                                courier1_next_stop_location_lon = encounter_two_couriers[0].lon
                            else:
                                if len(encounter_two_couriers[0].route_flag_to_be_updated[T + 1]) > 1  and encounter_two_couriers[0].route_flag_to_be_updated[T + 2] != []:
                                    courier1_next_stop_order_loc_id = -1
                                    detect_time_added = 1
                                    while courier1_next_stop_order_loc_id == -1 and encounter_two_couriers[0].route_flag_to_be_updated[T + detect_time_added + 1] != []:
                                        for flag_item in encounter_two_couriers[0].route_flag_to_be_updated[T + detect_time_added]:
                                            for next_time_flag_item in \
                                                    encounter_two_couriers[0].route_flag_to_be_updated[T + detect_time_added + 1]:
                                                if flag_item == next_time_flag_item:
                                                    courier1_next_stop_order_loc_id = next_time_flag_item[0]
                                ...
                                if courier1_next_stop_order_loc_id > len(encounter_two_couriers[0].available_loc_real_time):
                                    print('debug')
                            ...
                            if T + 1 not in encounter_two_couriers[1].route_flag_to_be_updated.keys() or \
                                    encounter_two_couriers[1].cur_order_num == 0 or len(encounter_two_couriers[1].route_flag_to_be_updated[T + 1]) == 0:
                                courier2_next_stop_location_lat = encounter_two_couriers[1].lat
                                courier2_next_stop_location_lon = encounter_two_couriers[1].lon
                            else:
                                if len(encounter_two_couriers[1].route_flag_to_be_updated[T + 1]) > 1  and encounter_two_couriers[1].route_flag_to_be_updated[T + 2] != []:
                                    courier2_next_stop_order_loc_id = -1
                                    detect_time_added = 1
                                    while courier2_next_stop_order_loc_id == -1 and encounter_two_couriers[1].route_flag_to_be_updated[T + detect_time_added + 1] != []:
                                        for flag_item in encounter_two_couriers[1].route_flag_to_be_updated[T + detect_time_added]:
                                            for next_time_flag_item in \
                                                    encounter_two_couriers[1].route_flag_to_be_updated[T + detect_time_added + 1]:
                                                if flag_item == next_time_flag_item:
                                                    courier2_next_stop_order_loc_id = next_time_flag_item[0]
                                    ...
                                    courier2_next_stop_order_loc_id = \
                                    encounter_two_couriers[1].route_flag_to_be_updated[T + 1][0][0]
                                if courier2_next_stop_order_loc_id > len(encounter_two_couriers[1].available_loc_real_time):
                                    print('debug')
                    ...
                    critic.store_transition(np.array(state_input), chosen_courier_id, reward, np.array(next_s))
    ...
    # if '12running_reward_ep' not in globals():
    #     running_reward_ep = ep_rs_sum1
    # else:
    #     running_reward_ep = running_reward_ep * 0.95 + ep_rs_sum1 * 0.05
    # print("episode:", global_step, "  reward:", int(running_reward_ep))
